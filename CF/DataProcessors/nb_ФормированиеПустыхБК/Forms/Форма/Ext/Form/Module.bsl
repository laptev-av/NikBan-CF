...
Процедура СформироватьБКНаСервере(НомерБК, Штрихкод = Неопределено)
	...
	Попытка
		...
		КонецЕсли;	
		СтрокаНомерБК = nb_ПрограммаЛояльностиДружбаСервер.ПолучитьСтроковоеПредставлениеНомераБК(НомерБК);
		////////// ФИЗЛИЦО
		
		НовыйФизЛицо = Справочники.ФизическиеЛица.СоздатьЭлемент();
		НовыйФизЛицо.Родитель = nb_ПрограммаЛояльностиДружбаСервер.ПолучитьГруппуУчастниковПрограммыДружба();
		...
		НовыйДисконтнаяКарта.Родитель 			= nb_ПрограммаЛояльностиДружбаСервер.ПолучитьГруппуДисконтнойКартыПрограммыДружба();
		...
		НовыйДисконтнаяКарта.ВидДисконтнойКарты = nb_ПрограммаЛояльностиДружбаСервер.ПолучитьВидДисконтнойКартыПрограммыДружба();
		НовыйДисконтнаяКарта.БонуснаяПрограммаЛояльности = nb_ПрограммаЛояльностиДружбаСервер.ПолучитьБонуснуюПрограммуДружба();
