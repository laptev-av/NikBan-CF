	...
	Для Каждого СтрокаПриложение Из СписокПриложений Цикл
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(ВидТранспорта)
				...
				Если ТипЗнч(СтрокаПриложение.УзелИнформационнойБазы) = Тип("ПланОбменаСсылка.ПоМагазину") 
					ИЛИ ТипЗнч(СтрокаПриложение.УзелИнформационнойБазы) = Тип("ПланОбменаСсылка.ПоРабочемуМесту") Тогда
					СтрокаПриложение.ЗапускОбменаИзКорреспондента = Ложь;
				Иначе
