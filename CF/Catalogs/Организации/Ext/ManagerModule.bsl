...
//
// Возвращаемое значение  - Булево - продажа облагается\не облагается НДС.
Функция НалогообложениеНДС(Организация, Дата, Склад = Неопределено) Экспорт
	...
	|ГДЕ
	|	ВидыОсобыхНалоговОрганизаций.ВидНалога = ЗНАЧЕНИЕ(Перечисление.ВидыНалогов.ЕНВД)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ПолучитьСистемуНалогообложения(Дата, Организация) Экспорт
	...
	Иначе
		...
		Возврат Новый Структура("Дата, СистемаНалогообложения", "00010101", Перечисления.СистемыНалогообложения.Общая); // по умолчанию
		
	КонецЕсли;
	
КонецФункции

// Возвращает признак применения ЕНВД в организации.
Функция ПолучитьПрименениеЕНВД(Дата, Организация) Экспорт
	...
	|ИЗ
	|	РегистрСведений.ВидыОсобыхНалоговОрганизаций.СрезПоследних(&Дата, Организация = &Организация) КАК ВидыОсобыхНалоговОрганизаций
	|ГДЕ
	|	ВидыОсобыхНалоговОрганизаций.ВидНалога = ЗНАЧЕНИЕ(Перечисление.ВидыНалогов.ЕНВД)");
	Запрос.УстановитьПараметр("Дата", Дата);
	Запрос.УстановитьПараметр("Организация", Организация);
		
	ЕстьПрименениеЕНВД = Ложь;
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ЕстьПрименениеЕНВД = Истина;
	КонецЕсли;
	
	Возврат ЕстьПрименениеЕНВД;

КонецФункции

// Возвращает ставку НДС с учетом налогообложения организации.
Функция ПолучитьСтавкуНДС(Дата, Организация, Склад, Номенклатура = Неопределено) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Номенклатура) Тогда
			...
			СтавкаНДСВНоменклатуре = Номенклатура.СтавкаНДС;
			СтавкаНДС = УчетНДС.СкорректироватьСтавкуНДС(СтавкаНДСВНоменклатуре, Дата);
