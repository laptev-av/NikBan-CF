...
Процедура УстановитьРасписаниеПроверкиКэшаСостояний() Экспорт
	...
	Задание = РегламентныеЗаданияСервер.Задание(Метаданные.РегламентныеЗадания.ПроверкаКэшаСостоянийФНС);
...
Функция ПолучитьСостоянияКонтрагентов(Прокси, КонтрагентыЗапроса)
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'NdsRequest2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СообщениеЖурналаРегистрации,
			НСтр("ru = 'Ошибка доступа к веб-сервису'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ПредставлениеДатыВСервисе(Знач Дата)
	...
	НаТекущийДень = (ТипЗнч(Дата) = Тип("Строка") И Дата = Формат(ТекущаяДатаПК, "ДФ=dd.MM.yyyy")
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'ПроверкаКонтрагентов.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
