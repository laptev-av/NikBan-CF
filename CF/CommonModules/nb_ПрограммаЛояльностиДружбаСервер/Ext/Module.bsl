...
Функция ПолучитьДисконтнуюКартуПоБК(ВладелецКарты, НомерБК) Экспорт
	...
	Запрос.УстановитьПараметр("КодКарты", nb_НастройкиСервер.ПолучитьСтроковоеПредставлениеНомераБК(НомерБК));
...
Процедура ЗаполнитьСтрокуСкидкиДляРасчетаБонусов(КоличествоБонусов, ОтКогоНачисленыБонусныеБаллы = Неопределено, ДисконтнаяКарта, СтрокаДереваСкидок, ПараметрыБонуса, ТоварСегмента, ТаблицаСкидок)
		...
		БонуснаяПрограммаЛояльности = nb_НастройкиСервер.ПолучитьБонуснуюПрограммуДружба();
...
Процедура РассчитатьБонусныеБаллыПоПрограммеДружбаПоТоварам(ДисконтнаяКарта, СтрокаДереваСкидок, ПараметрыБонуса, ТоварыСегмента, ТаблицаСкидок) Экспорт
	...
	Для Каждого ТоварСегмента Из ТоварыДляРасчетаБонусов Цикл
		...
		Если ЗначениеЗаполнено(ДисконтнаяКарта) Тогда
			ТочностьОкругления1 = nb_НастройкиСервер.ПолучитьТочностьОкругленияПриРасчетеБонусныхБаллов(1);
			БонусныеБаллы1 = Окр(ТоварСегмента.Сумма * nb_НастройкиСервер.ПолучитьПроцентУровняДляНачисленияБонусныхБаллов(1) / 100, ТочностьОкругления1);
		...
		Если ЗначениеЗаполнено(ВладелецДисконтнойКарты2) Тогда
			ТочностьОкругления2 = nb_НастройкиСервер.ПолучитьТочностьОкругленияПриРасчетеБонусныхБаллов(2);
			БонусныеБаллы2 = Окр(ТоварСегмента.Сумма * nb_НастройкиСервер.ПолучитьПроцентУровняДляНачисленияБонусныхБаллов(2) / 100, ТочностьОкругления2);
		...
		Если ЗначениеЗаполнено(ВладелецДисконтнойКарты3) Тогда
			ТочностьОкругления3 = nb_НастройкиСервер.ПолучитьТочностьОкругленияПриРасчетеБонусныхБаллов(3);
			БонусныеБаллы3 = Окр(ТоварСегмента.Сумма * nb_НастройкиСервер.ПолучитьПроцентУровняДляНачисленияБонусныхБаллов(3) / 100, ТочностьОкругления3);
...
Процедура РассчитатьБонусныеБаллыПоПрограммеДружба(Знач ТаблицаПродажиПоДисконтнымКартам, ТаблицаБонусныхБаллов) Экспорт
	...
	Для Каждого СтрокаТЗ Из ТаблицаПродажиПоДисконтнымКартам Цикл 
		...
		Если ЗначениеЗаполнено(ВладелецДисконтнойКарты1) Тогда
			ТочностьОкругления1 = nb_НастройкиСервер.ПолучитьТочностьОкругленияПриРасчетеБонусныхБаллов(1);
			БонусныеБаллы1 = Окр(СтрокаТЗ.Сумма * nb_НастройкиСервер.ПолучитьПроцентУровняДляНачисленияБонусныхБаллов(1) / 100,ТочностьОкругления1);
			...
			СтрокаТЗ_БонусныхБаллов.БонуснаяПрограммаЛояльности		= nb_НастройкиСервер.ПолучитьБонуснуюПрограммуДружба();
		...
		Если ЗначениеЗаполнено(ВладелецДисконтнойКарты2) Тогда
			ТочностьОкругления2 = nb_НастройкиСервер.ПолучитьТочностьОкругленияПриРасчетеБонусныхБаллов(2);
			БонусныеБаллы2 = Окр(СтрокаТЗ.Сумма * nb_НастройкиСервер.ПолучитьПроцентУровняДляНачисленияБонусныхБаллов(2) / 100,ТочностьОкругления2);
			ДК2 = ПолучитьДисконтнуюКартуПоБК(ВладелецДисконтнойКарты2, ВладелецДисконтнойКарты2.nb_БК); 
			СтрокаТЗ_БонусныхБаллов = ТаблицаБонусныхБаллов.Добавить();
		...
		Если ЗначениеЗаполнено(ВладелецДисконтнойКарты3) Тогда
			ТочностьОкругления3 = nb_НастройкиСервер.ПолучитьТочностьОкругленияПриРасчетеБонусныхБаллов(3);
			БонусныеБаллы3 = Окр(СтрокаТЗ.Сумма * nb_НастройкиСервер.ПолучитьПроцентУровняДляНачисленияБонусныхБаллов(3) / 100,ТочностьОкругления3);
			ДК3 = ПолучитьДисконтнуюКартуПоБК(ВладелецДисконтнойКарты3, ВладелецДисконтнойКарты3.nb_БК); 
			СтрокаТЗ_БонусныхБаллов = ТаблицаБонусныхБаллов.Добавить();
...
Функция ПолучитьАналитикуНачисленияБонусов(Период) Экспорт
	...
	Запрос.УстановитьПараметр("Период", Период);
	Запрос.УстановитьПараметр("БонуснаяПрограммаЛояльности", nb_НастройкиСервер.ПолучитьБонуснуюПрограммуДружба());
