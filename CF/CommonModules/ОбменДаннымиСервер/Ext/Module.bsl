...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	ВремяСинхронизации = Формат(ДатаСинхронизации, "ДЛФ=T");
	...
	ИначеЕсли Интервал < 60 * 5 Тогда // 5 мин
		...
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сейчас (%1)'"), ВремяСинхронизации);
		
	ИначеЕсли Интервал < 60 * 15 Тогда // 15 мин
		
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '5 минут назад (%1)'"), ВремяСинхронизации);
		
	ИначеЕсли Интервал < 60 * 30 Тогда // 30 мин
		
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '15 минут назад (%1)'"), ВремяСинхронизации);
		
	ИначеЕсли Интервал < 60 * 60 * 1 Тогда // 1 час
		
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '30 минут назад (%1)'"), ВремяСинхронизации);
		
	ИначеЕсли Интервал < 60 * 60 * 2 Тогда // 2 часа
		
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '1 час назад (%1)'"), ВремяСинхронизации);
		
	ИначеЕсли Интервал < 60 * 60 * 3 Тогда // 3 часа
		
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '2 часа назад (%1)'"), ВремяСинхронизации);
	...
	Иначе
		...
		Если КоличествоДнейРазницы = 0 Тогда // сегодня
			...
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сегодня, %1'"), ВремяСинхронизации);
			
		ИначеЕсли КоличествоДнейРазницы = 1 Тогда // вчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вчера, %1'"), ВремяСинхронизации);
			
		ИначеЕсли КоличествоДнейРазницы = 2 Тогда // позавчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Позавчера, %1'"), ВремяСинхронизации);
...
Функция ЕстьПраваНаАдминистрированиеОбменов() Экспорт
	...
	// Функция изменена. Добавлена проверка на доступность роли ДобавлениеИзменениеУзловПлановОбмена,
	// необходимо для работы функциона по проекту Обмены под неполными правами.
	Если Пользователи.РолиДоступны("ДобавлениеИзменениеУзловПлановОбмена") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
