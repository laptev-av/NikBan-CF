...
Функция НачатьЗамер(АвтоЗавершение = Истина, Событие, ТолькоНаКлиенте = Истина) Экспорт
	...
	Если ТолькоНаКлиенте Тогда		
		...
		Замер.Идентификатор = Замер.МодульОценкаПроизводительностиКлиент.НачатьЗамерВремени(АвтоЗавершение, Событие);
		Замер.МодульОценкаПроизводительностиКлиент.УстановитьКомментарийЗамера(Замер.Идентификатор, "");
	Иначе
		Замер = МодульОценкаПроизводительностиКлиент.НачатьЗамерВремени(АвтоЗавершение, Событие);
	КонецЕсли;
