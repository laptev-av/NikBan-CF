...
Процедура ОбработатьСтрокуТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ПроверитьНеобходимостьВводаКодаМаркировки(ТекущаяСтрока                  , СтруктураДействий, КэшированныеЗначения);
...
Процедура ЗаполнитьСтавкуНДСВСтрокеСкладВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуНДССкладВСтроке", СтруктураПараметровДействия) Тогда
		...
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	СистемыНалогообложенияОрганизацийСрезПервых.Организация КАК Организация
		|ПОМЕСТИТЬ ТаблицаОрганизацииНаУпрощеннойСистеме
		|ИЗ
		|	РегистрСведений.СистемыНалогообложенияОрганизаций.СрезПоследних(&Дата, СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)) КАК СистемыНалогообложенияОрганизацийСрезПервых
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|				И Склад = &Склад
		|				И НЕ &ЭтоУслуга) КАК ВидыОсобыхНалоговОрганизаций
		|ГДЕ
		|	ВидыОсобыхНалоговОрганизаций.ВидНалога В(&МассивВидовНалогаНеоблагаемыхНДС)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ИЛИ Налогообложение.НеЯвляетсяПлательщикомНДС)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И РаспределениеПродажУслугПоОрганизациям.ВидНалога В(&МассивВидовНалогаНеоблагаемыхНДС)
		|	И &ЭтоУслуга
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	РаспределениеПродажУслугПоОрганизациям.Магазин = &Магазин
		|	И РаспределениеПродажУслугПоОрганизациям.ТоварнаяГруппа = &ТоварнаяГруппа
		|	И РаспределениеПродажУслугПоОрганизациям.Организация В
		|			(ВЫБРАТЬ
		|				ТаблицаОрганизацииНаУпрощеннойСистеме.Организация КАК Организация
		|			ИЗ
		|				ТаблицаОрганизацииНаУпрощеннойСистеме КАК ТаблицаОрганизацииНаУпрощеннойСистеме)");
		
		Если СтруктураПараметровДействия.Свойство("Склад") Тогда
			Склад = СтруктураПараметровДействия.Склад;
		Иначе
		...
		Если СтруктураПараметровДействия.Свойство("Магазин") Тогда
		...
		РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Номенклатура, "ТоварнаяГруппа, СтавкаНДС, ТипНоменклатуры");
		Запрос.УстановитьПараметр("ТоварнаяГруппа", РеквизитыНоменклатуры.ТоварнаяГруппа);
		Запрос.УстановитьПараметр("ЭтоУслуга", РеквизитыНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга);
		
		МассивВидовНалогаНеоблагаемыхНДС = Новый Массив;
		МассивВидовНалогаНеоблагаемыхНДС.Добавить(Перечисления.ВидыНалогов.ЕНВД);
		МассивВидовНалогаНеоблагаемыхНДС.Добавить(Перечисления.ВидыНалогов.ПСН);
		
		Если СтруктураПараметровДействия.Дата < УчетНДС.ДатаПереходногоПериода() Тогда
			МассивВидовНалогаНеоблагаемыхНДС.Добавить(Перечисления.ВидыНалогов.ЕСХН);
		КонецЕсли;
		
		Запрос.УстановитьПараметр("МассивВидовНалогаНеоблагаемыхНДС", МассивВидовНалогаНеоблагаемыхНДС);
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
		Иначе
			СтавкаНДСВНоменклатуре = РеквизитыНоменклатуры.СтавкаНДС;
			СтавкаНДС = УчетНДС.СкорректироватьСтавкуНДС(СтавкаНДСВНоменклатуре, СтруктураПараметровДействия.Дата);
...
Процедура ЗаполнитьСтавкуНДСВСтрокеСкладВШапкеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуНДССкладВШапке", СтруктураПараметровДействия) Тогда
		СтруктураДействий.Вставить("ЗаполнитьСтавкуНДССкладВСтроке", СтруктураПараметровДействия);
		ЗаполнитьСтавкуНДСВСтрокеСкладВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	КонецЕсли;
...
// Процедура пересчета "ПересчитатьСуммуПродажи"
...
// Процедура пересчета "ЗаполнитьСтрануПроисхождения"
//
...
Процедура ЗаполнитьСкладПродажиВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСкладПродажи", СтруктураПараметровДействия) Тогда
		Если ТекущаяСтрока.Свойство("ТипНоменклатуры") Тогда
		...
		Иначе
			РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Номенклатура, "ТипНоменклатуры, ТоварнаяГруппа");
		...
		Если ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Услуга Тогда
			...
			|			ИЛИ РаспределениеПродаж.ТоварнаяГруппа = &ПустаяГруппа)
			|	И (РаспределениеПродаж.РабочееМесто = &РабочееМесто
			|			ИЛИ РаспределениеПродаж.РабочееМесто = &ПустоеМесто)
			|
			|УПОРЯДОЧИТЬ ПО
			|	ПолеУпорядочивания");
			...
			Запрос.УстановитьПараметр("ПустаяГруппа", Справочники.ТоварныеГруппы.ПустаяСсылка());
			...
			Иначе
				ТекущаяСтрока.Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметровДействия.Магазин, "СкладПродажи");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьОрганизациюПродажиВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьОрганизациюПродажи", СтруктураПараметровДействия) Тогда
		...
		Иначе
			ТекущаяСтрока.Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Склад, "Организация");
		КонецЕсли;
		
		Если (НЕ ЗначениеЗаполнено(ТекущаяСтрока.Организация))
			И ЗначениеЗаполнено(СтруктураПараметровДействия.Магазин) Тогда
			ТекущаяСтрока.Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметровДействия.Магазин, "СкладПродажи.Организация");
		КонецЕсли;
...
Процедура ПроверитьОтличныеДоговорыПлатежныхАгентов(ТекущаяСтрока, СтруктураДействий)
	...
	Если Не СтруктураПараметровДействия.ДоговорПлатежногоАгента = ТекущаяСтрока.ДоговорПлатежногоАгента Тогда
		
		ТекстСообщения = СтруктураПараметровДействия.ТекстСообщения;
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ТекущаяСтрока.Номенклатура);
		ТекущаяСтрока.Номенклатура = Справочники.Номенклатура.ПустаяСсылка();
		ТекущаяСтрока.ДоговорПлатежногоАгента = Справочники.ДоговорыПлатежныхАгентов.ПустаяСсылка();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
//
Процедура УдалитьНесвязанныеКодыМаркировки(Форма) Экспорт
	
	Ключи = Форма.Объект.КодыМаркировки.Выгрузить();
	Ключи.Свернуть("КлючСвязи");
	Для Каждого СтрокаКлюча ИЗ Ключи Цикл
		СтруктураКлюча = Новый Структура;
		СтруктураКлюча.Вставить("КлючСвязи", СтрокаКлюча.КлючСвязи);
		СтрокиТоваров = Форма.Объект.Товары.НайтиСтроки(СтруктураКлюча);
		Если СтрокиТоваров.Количество() = 0 Тогда
			УдаляемыеСтроки = Форма.Объект.КодыМаркировки.НайтиСтроки(СтруктураКлюча);
			Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
				Форма.Объект.КодыМаркировки.Удалить(УдаляемаяСтрока);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьНеобходимостьВводаАкцизнойМарки(ТаблицаФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	|	ТаблицаЗапроса.Номенклатура.ВидАлкогольнойПродукцииЕГАИС.Маркируемый И НЕ ТаблицаЗапроса.Номенклатура.ВидНоменклатуры.ПродаетсяВРозлив КАК НеобходимостьВводаАкцизнойМарки
	|ИЗ
	|	ТаблицаЗапроса КАК ТаблицаЗапроса";
	...
	Для Каждого СтрТабл Из ТаблицаФормы Цикл
		...
		Если ИспользуетсяРегистрацияРозничныхПродажВЕГАИС = Неопределено Тогда
			Если МассивОрганизацийИспользующихРозничныеПродажиЕГАИС.Найти(СтрТабл.Организация) = Неопределено Тогда
				СтрТабл.НеобходимостьВводаАкцизнойМарки = Ложь;
			Иначе
...
Процедура ЗаполнитьНеобходимостьВводаКодаМаркировки(ТаблицаФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОперацияСДенежнымиСредствами Тогда
		Для Каждого СтрокаТаблицы Из ТаблицаФормы Цикл
			СтрокаТаблицы.НеобходимостьВводаКодаМаркировки = Ложь;
		КонецЦикла;
	Иначе
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ТаблицаЗапроса.Номенклатура.ТабачнаяПродукция, ЛОЖЬ)
		|				ИЛИ ЕСТЬNULL(ТаблицаЗапроса.Номенклатура.ОбувнаяПродукция, ЛОЖЬ)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		...
		Для Каждого СтрокаТаблицы Из ТаблицаФормы Цикл
			...
			СтрокаТаблицы.НеобходимостьВводаКодаМаркировки = СтрокаРезультата.НеобходимостьВводаКодаМаркировки;
		КонецЦикла;
...
Процедура ПроверитьНеобходимостьВводаКодаМаркировки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаКодаМаркировки") Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда 
			...
			Если СтруктураДействий.ПроверитьНеобходимостьВводаКодаМаркировки <> Неопределено
				И СтруктураДействий.ПроверитьНеобходимостьВводаКодаМаркировки.Свойство("ОперацияСДенежнымиСредствами") Тогда
				ОперацияСДенежнымиСредствами = СтруктураДействий.ПроверитьНеобходимостьВводаКодаМаркировки.ОперацияСДенежнымиСредствами;
			КонецЕсли;
			...
			Если (НЕ ОперацияСДенежнымиСредствами)
				И (ВестиУчетМаркировкиТабачнойПродукции Или ВестиУчетМаркировкиОбувнойПродукции) Тогда
				
				СтруктураМаркировки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Номенклатура, "ТабачнаяПродукция, ОбувнаяПродукция");
				НеобходимВводКодаМаркировки = Ложь;
				
				Если ВестиУчетМаркировкиТабачнойПродукции И СтруктураМаркировки.ТабачнаяПродукция Тогда 
					НеобходимВводКодаМаркировки = Истина;
				ИначеЕсли ВестиУчетМаркировкиОбувнойПродукции И СтруктураМаркировки.ОбувнаяПродукция Тогда 
					НеобходимВводКодаМаркировки = Истина;
				КонецЕсли;
				
				ТекущаяСтрока.НеобходимостьВводаКодаМаркировки = НеобходимВводКодаМаркировки;
	...
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаАкцизнойМарки", СтруктураПараметровДействия) Тогда
		...
		Если ТипЗнч(СтруктураПараметровДействия) = Тип("Структура") И СтруктураПараметровДействия.Свойство("Магазин") Тогда
			...
			Если не ОрганизацияПроверки = Неопределено Тогда
				ИспользуетсяРегистрацияРозничныхПродажВЕГАИС = ИнтеграцияЕГАИСВызовСервера.ИспользуетсяРегистрацияРозничныхПродажВЕГАИС(ОрганизацияПроверки, МагазинПроверки, ПроверяемаяДата);
			КонецЕсли;
		...
		Если ИспользуетсяРегистрацияРозничныхПродажВЕГАИС И НЕ ОперацияСДенежнымиСредствами Тогда
			...
			Если ДанныеПродукции.ЭтоАлкогольнаяПродукция Тогда
				...
				Если ДанныеПродукции.Маркируемый
...
// Процедура заполнения вида налога в строках.
//
Процедура ЗаполнитьВидНалогаПродажиВСтрокеТЧСервер(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьВидНалога", СтруктураПараметровДействия) Тогда
		...
		Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
			...
			ОсновнойСкладПродажи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметровДействия.Магазин, "СкладПродажи");
			ОрганизацияОсновногоСкладаПродажи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнойСкладПродажи, "Организация");
			
			Запрос = Новый Запрос();
			Запрос.Текст = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	РаспределениеПродажУслугПоОрганизациям.ВидНалога,
			...
			|
			|ВЫБРАТЬ
			|	ВидыОсобыхНалоговОрганизаций.ВидНалога,
			|	2
			|ИЗ
			|	РегистрСведений.ВидыОсобыхНалоговОрганизаций.СрезПоследних(
			|			&Дата,
			|			Организация = &ОрганизацияОсновногоСкладаПродажи
			|				И Склад = &ОсновнойСкладПродажи) КАК ВидыОсобыхНалоговОрганизаций
			|
			|УПОРЯДОЧИТЬ ПО
			|	ПолеУпорядочивания";
			Запрос.УстановитьПараметр("Дата", СтруктураПараметровДействия.Дата);
			Запрос.УстановитьПараметр("Магазин", СтруктураПараметровДействия.Магазин);
			Запрос.УстановитьПараметр("ТоварнаяГруппа", ТоварнаяГруппа);
			Запрос.УстановитьПараметр("ОрганизацияОсновногоСкладаПродажи", ОрганизацияОсновногоСкладаПродажи);
			Запрос.УстановитьПараметр("ОсновнойСкладПродажи", ОсновнойСкладПродажи);
			
			Результат = Запрос.Выполнить();
			Выборка = Результат.Выбрать();
			
			Если Выборка.Следующий() Тогда
				ТекущаяСтрока.ВидНалога = Выборка.ВидНалога;
			Иначе
				ТекущаяСтрока.ВидНалога = Перечисления.ВидыНалогов.ПустаяСсылка();
			КонецЕсли;
...
// Процедура заполнения вида налога в строках.
//
Процедура ЗаполнитьВидНалогаПродажиВТЧСервер(ТаблицаФормы, СтруктураПараметров) Экспорт

	Если ТаблицаФормы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОсновнойСкладПродажи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметров.Магазин, "СкладПродажи");
	ОрганизацияОсновногоСкладаПродажи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнойСкладПродажи, "Организация");
	...
	|	ТаблицаЗапроса.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР
	|		КОГДА ТаблицаЗапроса.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			ТОГДА ВЫБОР
	|					КОГДА РаспределениеПродажУслугПоОрганизациям.ВидНалога ЕСТЬ NULL
	|						ТОГДА РаспределениеПродажУслугПоОрганизациямПоУмолчанию.ВидНалога
	|					ИНАЧЕ РаспределениеПродажУслугПоОрганизациям.ВидНалога
	|				КОНЕЦ
	|		ИНАЧЕ ВидыОсобыхНалоговОрганизаций.ВидНалога
	|	КОНЕЦ КАК ВидНалога
	|ИЗ
	|	ТаблицаЗапроса КАК ТаблицаЗапроса
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВидыОсобыхНалоговОрганизаций.СрезПоследних(&Дата, ) КАК ВидыОсобыхНалоговОрганизаций
	|		ПО ТаблицаЗапроса.Организация = ВидыОсобыхНалоговОрганизаций.Организация
	|			И ТаблицаЗапроса.Склад = ВидыОсобыхНалоговОрганизаций.Склад
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеПродажУслугПоОрганизациям КАК РаспределениеПродажУслугПоОрганизациям
	|		ПО ТаблицаЗапроса.Магазин = РаспределениеПродажУслугПоОрганизациям.Магазин
	|			И ТаблицаЗапроса.Номенклатура.ТоварнаяГруппа = РаспределениеПродажУслугПоОрганизациям.ТоварнаяГруппа
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВидыОсобыхНалоговОрганизаций.СрезПоследних(
	|				&Дата,
	|				Организация = &ПоУмолчаниюОрганизация
	|					И Склад = &СкладПоУмолчанию) КАК РаспределениеПродажУслугПоОрганизациямПоУмолчанию
	|		ПО (ИСТИНА)
	|ГДЕ
	|	НЕ ВЫБОР
	|				КОГДА ТаблицаЗапроса.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	...
	Запрос.УстановитьПараметр("ТаблицаТоваров", ТаблицаФормы.Выгрузить());
	Запрос.УстановитьПараметр("Магазин", СтруктураПараметров.Магазин);
	Запрос.УстановитьПараметр("ПоУмолчаниюОрганизация", ОрганизацияОсновногоСкладаПродажи);
	Запрос.УстановитьПараметр("СкладПоУмолчанию", ОсновнойСкладПродажи);
	...
	Для Каждого СтрокаФормы Из ТаблицаФормы Цикл
		
		СтруктураПоиска = Новый Структура("НомерСтроки", СтрокаФормы.НомерСтроки);
		НайденныеСтроки = Результат.НайтиСтроки(СтруктураПоиска);
		
		Если НайденныеСтроки.Количество() > 0 Тогда
			СтрокаРезультата = НайденныеСтроки[0];
			СтрокаФормы.ВидНалога = СтрокаРезультата.ВидНалога;
		Иначе
			СтрокаФормы.ВидНалога = Перечисления.ВидыНалогов.ПустаяСсылка();
		КонецЕсли;
...
Процедура ЗаполнитьПризнакМаркируемаяАлкогольнаяПродукция(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакМаркируемаяАлкогольнаяПродукция", Параметры) Тогда
		...
		Номенклатура = Параметры.Номенклатура;
		МаркируемаяАлкогольнаяПродукция = Параметры.МаркируемаяАлкогольнаяПродукция;
...
Функция ДанныеЕдиницыИзмренияВЕТИС(ЕдиницаИзмеренияВЕТИС, Номенклатура, КэшированныеЗначения) Экспорт
	...
	ДанныеЕдиницыИзмренияВЕТИС = Справочники.БазовыеЕдиницыИзмерения.КоэффициентЕдиницыИзмеренияПоВЕТИС(
									ЕдиницаИзмеренияВЕТИС, Номенклатура);
	
	Если ДанныеЕдиницыИзмренияВЕТИС.КэшироватьДанные Тогда
	...
	Возврат ДанныеЕдиницыИзмренияВЕТИС;
