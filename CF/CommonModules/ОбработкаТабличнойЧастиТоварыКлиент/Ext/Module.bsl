...
Процедура ПриИзмененииРеквизитовВТЧКлиент(ТЧ, ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ПроверятьДробноеКоличество = ОбработкаТабличнойЧастиТоварыВызовСервера.ПроверитьИспользованиеВводаДробногоКоличества();	
...
Процедура ОбработатьСтрокуТЧСВызовомСервера(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	//КонецЕсли;
		...
		СтруктураПолей = ПолучитьСтруктуруПолейТЧ(СтруктураДействий);

		СтрокаДляСервера = ПолучитьДанныеТекущейСтроки(ТекущаяСтрока, СтруктураПолей);
...
Процедура ЗаполнитьВидНалогаПоСкладуПродажиВСтрокеТЧКлиент(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьВидНалога") Тогда
		ТекущаяСтрока.ВидНалога = ПредопределенноеЗначение("Перечисление.ВидыНалогов.ПустаяСсылка");
	КонецЕсли;
		...
		Если ОбработкаТабличнойЧастиТоварыВызовСервера.ПроверитьИспользованиеХарактеристикИПолучитьВладельцаДляВыбора(ТекущаяСтрока.Номенклатура, ВладелецХарактеристики) Тогда
			...
			Иначе
				...
				Если ОткрытьСозданиеНовойХарактеристики Тогда
					...
					ПараметыСоздания = Новый Структура;
					ПараметыСоздания.Вставить("ИдентификаторТекущейСтроки", ТекущаяСтрока.ПолучитьИдентификатор());
					ПараметыСоздания.Вставить("ИмяТЧ", ИмяТЧ);
					ОбработчикОповещения = Новый ОписаниеОповещения("ОбработатьСозданиеИВыборНовойХарактеристики", Форма, ПараметыСоздания);
		...
		Если ОбработкаТабличнойЧастиТоварыВызовСервера.ПроверитьИспользованиеХарактеристикИПолучитьВладельцаДляВыбора(ТекущаяСтрока.Номенклатура, ВладелецХарактеристики) Тогда
			...
			Иначе
				...
				Если ТипЗнч(ТекущаяСтрока) = Тип("Структура") Тогда
					ПараметыСоздания.Вставить("ИдентификаторТекущейСтроки", ТекущаяСтрока.ИдентификаторТекущейСтроки);
				Иначе
					ПараметыСоздания.Вставить("ИдентификаторТекущейСтроки", ТекущаяСтрока.ПолучитьИдентификатор());
				КонецЕсли;
				
				ПараметыСоздания.Вставить("ИмяТЧ", ИмяТЧ);
				ПараметыСоздания.Вставить("ИмяРеквизита", ИмяРеквизита);
...
Функция ПолучитьПараметрыОбработкиТЧ(ТЧ, ТекущаяСтрока, СтруктураОбщихДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаКодаМаркировки")
		И ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		НеобходимВызовСервера = Истина;
	КонецЕсли;
...
Процедура ПроверитьНеобходимостьВводаКодаМаркировки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаКодаМаркировки") Тогда
		ТекущаяСтрока.НеобходимостьВводаКодаМаркировки = Ложь;
	КонецЕсли;
...
// Процедура проверяет возможность ввода дробного количества
//
