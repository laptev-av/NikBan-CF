...
Процедура НайтиБитыеСсылки(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	ОбъектМетаданныхСсылочный.Ссылка КАК ПроблемныйОбъект
	|ИЗ
	|	&ОбъектМетаданных КАК ОбъектМетаданныхСсылочный
	|ГДЕ
	|	ОбъектМетаданныхСсылочный.Ссылка > &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОбъектМетаданныхСсылочный.Ссылка";
...
Процедура НайтиЦиклическиеСсылки(ОбъектМетаданных, ПараметрыПроверки)
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	ОбъектМетаданныхСсылочный.Ссылка КАК ПроблемныйОбъект,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ОбъектМетаданныхСсылочный.Ссылка) КАК Представление,
	|	ОбъектМетаданныхСсылочный.Родитель КАК Родитель
	|ИЗ
	|	&ОбъектМетаданных КАК ОбъектМетаданныхСсылочный
	|ГДЕ
	|	ОбъектМетаданныхСсылочный.Ссылка > &Ссылка
...
Функция КоличествоДочернихЭлементов(СсылкаНаОбъект, ИсключениеВыборки, Знач НачальноеЗначение = 0)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ОбъектМетаданныхСсылочный.Ссылка КАК Ссылка
	|ИЗ
	|	&ОбъектМетаданных КАК ОбъектМетаданныхСсылочный
	|ГДЕ
	|	ОбъектМетаданныхСсылочный.Родитель = &Родитель
	|	И ОбъектМетаданныхСсылочный.Ссылка <> &ИсключениеВыборки";
