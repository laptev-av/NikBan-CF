...
&НаСервере
Функция ПолучитьАдресФайлаОтчетаВХранилище(ТипФайла = Неопределено)
	
	Если ТипФайла = Неопределено Тогда
		ТипФайла = nb_НастройкиСервер.ПолучитьТипФайла_СводныйОтчетПоСмене();
	КонецЕсли;
	
	АдресФайлаВХранилище = nb_ОтчетностьСервер.ПолучитьАдресФайлаОтчетаВХранилище_СводныйОтчетПоСмене(Объект.Ссылка, ТипФайла);
...
Процедура nb_nb_ОтправитьОтчетEMail()
	...
	АдресФайлаОтчета = ПолучитьАдресФайлаОтчетаВХранилище(ТипФайла);
	ИмяФайлаОтчета = "Report_"+Формат(Объект.Дата,"ДФ=ггггммддччммсс")+"."+nb_НастройкиСервер.ПолучитьРасширениеПоТипуФайла(ТипФайла);
	...
	АдресаПолучателя = nb_НастройкиСервер.ПолучитьАдресаEmailДляОтправки_СводныйОтчетПоСмене();
...
Процедура nb_nb_ОтправитьОтчетSMS()
	...
	НомераПолучателей = nb_НастройкиСервер.ПолучитьНомераДляОтправкиSMS_СводныйОтчетПоСмене();
